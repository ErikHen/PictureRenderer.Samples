...
namespace OptimizelyCMS.Infrastructure
{
    public class UsersInstaller : IBlockingFirstRequestInitializer
    {
        ...
        {
            var result = await _uIUserProvider.CreateUserAsync(username, "Optimizely123!", email, null, null, true);
            if (result.Status == UIUserCreateStatus.Success)
            {
                ...
                var resFromSignIn = await _uISignInManager.SignInAsync(username, "Optimizely123!");
            }
        }
